...
export default function MemoryLeaderboardPage() {
  const [entries, setEntries] = useState<Entry[] | null>(null);

  useEffect(() => {
    fetch("/api/memory/leaderboard")
      .then((res) => res.json())
    ...
    <div className="mx-auto flex w-full max-w-lg flex-1 flex-col px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-muted">Best single-player runs, fewest moves first.</p>
        </div>
        <Link
          href="/games/memory"
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-surface">
        ...
        {entries?.length === 0 && (
          <p className="p-6 text-center text-sm text-text-muted">
            No single-player runs yet. Be the first!
          </p>
        )}
        {entries?.map((entry, i) => (
          <div
          ...
          >
            ...
            <span className="text-right">{entry.moves}</span>
